...
interface SettingsModalProps {
  ...
  apiUsage: { defaultKey: number; userKey: number };
  onResetUsage: () => void;
}

export const SettingsModal: React.FC<SettingsModalProps> = ({ isOpen, onClose, onSaveApiKey, onClearApiKey, currentApiKey, apiUsage, onResetUsage }) => {
        ...
        <main className="p-4 sm:p-5 space-y-4">
            <div className="space-y-2">
                 ...
                 <p className="text-sm text-muted-foreground">
                    You can use your own Google AI Studio API key. This can be useful if the default key is rate-limited. Your key is saved securely in your browser's local storage.
                </p>
                <div>
             ...
             <div className="border-t border-border pt-4 space-y-3">
                <h3 className="text-sm font-semibold text-muted-foreground">API Usage Stats</h3>
                <div className="text-sm text-card-foreground space-y-1">
                    <p>Calls with Default Key: <span className="font-bold">{apiUsage.defaultKey}</span></p>
                    <p>Calls with Your Key: <span className="font-bold">{apiUsage.userKey}</span></p>
                </div>
                <button onClick={onResetUsage} className="text-xs text-muted-foreground underline hover:text-foreground">
                    Reset Stats
                </button>
             </div>
        </main>
